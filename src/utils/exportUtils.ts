...
import {
  formatExportSupplierValue,
  sanitizeFindingReason,
  isDocumentDump,
} from './sanitize';

// Re-export so existing external imports of `formatExportSupplierValue` from
// `exportUtils` (e.g. ReportModal.tsx) keep working unchanged.
export { formatExportSupplierValue };

/**
 * Last-line-of-defense guard applied at render time, immediately before a
 * clarification-point field is written into the PDF or Excel export. The
 * upstream draft (server.ts / ReportModal.tsx) is expected to have already
 * produced safe, parameter-specific text via `buildClarificationDescription`,
 * but a stale/cached `ExternalFeedbackDraft` record (created before this fix,
 * or hand-edited by a user) could still carry a raw document dump. Rather
 * than ever rendering that dump into the export, this collapses it to the
 * mandated fallback copy.
 */
function sanitizeClarificationText(value: string | undefined | null, fieldLabel: string): string {
  const text = String(value || '').trim();
  if (!text) return `Manual review required — no ${fieldLabel} was recorded for this item.`;
  if (isDocumentDump(text)) {
    return `Manual review required — automated extraction could not isolate a parameter-specific ${fieldLabel} from the submitted certificate.`;
  }
  const singleLine = text.replace(/[\r\n\t]+/g, ' ').trim();
  return singleLine.length > 400 ? `${singleLine.slice(0, 380)}... Manual review required.` : singleLine;
}
...
export function exportAnalysisToExcel(
  ...
  if (feedbackDraft) {
    ...
    feedbackDraft.clarificationPoints.forEach((pt, idx) => {
      // Defense-in-depth: never render a raw document dump into the export,
      // even if it slipped through from a stale/cached feedback draft.
      const safeDescription = sanitizeClarificationText(pt.description, 'finding description');
      const safeAction = sanitizeClarificationText(pt.actionRequired, 'required action');
      const pRow = [idx + 1, pt.title, safeDescription, safeAction];
      for (let c = 0; c < 4; c++) {
        wsDraft[XLSX.utils.encode_cell({ r: rIdx, c })] = {
          v: pRow[c],
...
export function exportAnalysisToPDF(
  ...
  const drawPageHeader = () => {
    // Official MTC Logo: Green rounded square with white document & checkmark
    const logoX = margin;
    const logoY = 6.2;
    const logoSize = 10;

    // 1. Emerald green rounded square container (#059669) with subtle border (#34D399)
    doc.setFillColor(5, 150, 105);
    doc.setDrawColor(52, 211, 153);
    doc.setLineWidth(0.35);
    doc.roundedRect(logoX, logoY, logoSize, logoSize, 2.2, 2.2, 'FD');

    // 2. White document sheet with top-right corner fold
    const docX = logoX + 2.4;
    const docY = logoY + 1.8;
    const docW = 5.2;
    const docH = 6.4;
    const fold = 1.6;
    ...
    // Document outline
    doc.line(docX, docY, docX + docW - fold, docY);
    doc.line(docX + docW - fold, docY, docX + docW, docY + fold);
    doc.line(docX + docW, docY + fold, docX + docW, docY + docH);
    doc.line(docX + docW, docY + docH, docX, docY + docH);
    doc.line(docX, docY + docH, docX, docY);

    // Corner fold crease
    doc.line(docX + docW - fold, docY, docX + docW - fold, docY + fold);
    doc.line(docX + docW - fold, docY + fold, docX + docW, docY + fold);

    // 3. Crisp white checkmark
    doc.setLineWidth(0.6);
    doc.line(docX + 1.2, docY + 3.8, docX + 2.3, docY + 4.9);
    doc.line(docX + 2.3, docY + 4.9, docX + 4.1, docY + 2.6);

    // Left: Application Title & Organization (offset to sit right of logo)
  ...
  if (_feedbackDraft && _feedbackDraft.clarificationPoints && _feedbackDraft.clarificationPoints.length > 0) {
    ...
    _feedbackDraft.clarificationPoints.forEach((pt, idx) => {
      ...
      const descLines = doc.splitTextToSize(`Description: ${safeDescription}`, contentWidth - 12);
      const actLines = doc.splitTextToSize(`Required Action: ${safeAction}`, contentWidth - 12);
